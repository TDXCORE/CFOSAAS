/*
 * -------------------------------------------------------
 * CFO SaaS Platform - Colombian Seed Data
...
('5195', 'GASTOS DIVERSOS', 'Otros gastos de administración no clasificados anteriormente', 3, '51', 'expense', 'debit', ARRAY['diversos', 'varios', 'otros'], ARRAY[], '{"deductible": true}'),
...
('5365', 'GASTOS EXTRAORDINARIOS', 'Comprende los gastos de carácter excepcional', 3, '53', 'expense', 'debit', ARRAY['extraordinario', 'excepcional'], ARRAY[], '{"deductible": false}')
...
('Servicios Legales', '5140', 
 '{"supplier_name_contains": ["notaria", "registro", "juridico"], "description_keywords": ["notarial", "registro", "legal"], "exclude_keywords": []}', 
 0.85, 90, 0.90),
...
-- Additional performance indexes for Colombian data
CREATE INDEX IF NOT EXISTS idx_puc_accounts_keywords_gin ON public.puc_accounts USING gin(keywords);
CREATE INDEX IF NOT EXISTS idx_puc_accounts_supplier_patterns_gin ON public.puc_accounts USING gin(supplier_patterns);
CREATE INDEX IF NOT EXISTS idx_tax_rules_priority_effective ON public.tax_rules(priority DESC, effective_from, effective_to);
CREATE INDEX IF NOT EXISTS idx_classification_rules_accuracy ON public.classification_rules(accuracy_rate DESC, priority DESC);
