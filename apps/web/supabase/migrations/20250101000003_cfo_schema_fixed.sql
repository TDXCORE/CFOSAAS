/*
 * -------------------------------------------------------
 * CFO SaaS Platform - Colombian Database Schema (Fixed)
 * Clean schema without IMMUTABLE function issues
 * -------------------------------------------------------
 */
...
/*
 * -------------------------------------------------------
 * Section: Colombian Tax System Tables
 * -------------------------------------------------------
 */
...
-- Add FK constraint for parent_code after table creation
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'puc_accounts_parent_code_fkey' 
        AND table_name = 'puc_accounts'
    ) THEN
...
-- Basic indexes for PUC accounts
...
-- Add FK constraint for puc_code after puc_accounts is populated
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM public.puc_accounts LIMIT 1) THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'classification_rules_puc_code_fkey' 
            AND table_name = 'classification_rules'
        ) THEN
            ALTER TABLE public.classification_rules 
            ADD CONSTRAINT classification_rules_puc_code_fkey 
            FOREIGN KEY (puc_code) REFERENCES public.puc_accounts(code);
        END IF;
    END IF;
END $$;
...
DO $$ BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_user_companies_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_ai_insights_updated_at') THEN
        CREATE TRIGGER update_ai_insights_updated_at 
        BEFORE UPDATE ON public.ai_insights 
        FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_chat_conversations_updated_at') THEN
...
DROP POLICY IF EXISTS "Users can only access their companies" ON public.companies;
...
DROP POLICY IF EXISTS "Users can only access their company relationships" ON public.user_companies;
...
DROP POLICY IF EXISTS "Users can only access company invoices" ON public.invoices;
...
DROP POLICY IF EXISTS "Users can only access company line items" ON public.invoice_line_items;
...
DROP POLICY IF EXISTS "Users can only access company taxes" ON public.invoice_taxes;
...
DROP POLICY IF EXISTS "Users can access classification rules" ON public.classification_rules;
...
DROP POLICY IF EXISTS "Users can only access company insights" ON public.ai_insights;
...
DROP POLICY IF EXISTS "Users can only access company conversations" ON public.chat_conversations;
...
DROP POLICY IF EXISTS "Users can only access their messages" ON public.chat_messages;
