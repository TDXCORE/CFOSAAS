/*
 * -------------------------------------------------------
 * CFO SaaS Platform - Colombian Database Schema
 * Database migration for invoice processing and AI CFO functionality
 * Focused on Colombian tax system and PUC classification
 * -------------------------------------------------------
 */
...
/*
 * -------------------------------------------------------
 * Section: Core Multi-tenant Tables
 * Companies and user relationships for Colombian businesses
 * -------------------------------------------------------
 */

-- Companies table (extends accounts for business focus)
CREATE TABLE IF NOT EXISTS public.companies (
  ...
  -- Link to accounts table for compatibility (optional)
  account_id UUID -- Will add FK constraint if accounts table exists
);
 ...
 * PUC accounts and tax rules specific to Colombia
 * -------------------------------------------------------
 */
...
CREATE TABLE IF NOT EXISTS public.puc_accounts (
  ...
  parent_code VARCHAR(10) REFERENCES public.puc_accounts(code),
 ...
 * Core invoice processing with Colombian tax calculations
 * -------------------------------------------------------
 */
...
-- Full-text search
CREATE INDEX IF NOT EXISTS idx_invoices_search ON public.invoices USING gin(
 ...
 * AI-generated insights and chat conversations
 * -------------------------------------------------------
 */
...
/*
 * -------------------------------------------------------
 * Section: Integration and Audit Tables
 * Tracking external integrations and audit trails
 * -------------------------------------------------------
 */
...
CREATE TRIGGER update_companies_updated_at 
  ...
  FOR EACH ROW EXECUTE FUNCTION kit.update_updated_at_column();
 ...
 * Multi-tenant security for Colombian CFO platform
 * -------------------------------------------------------
 */
...
-- Integration configs: Only owners/admins can access
CREATE POLICY "Admins can access integrations" ON public.integration_configs
FOR ALL USING (
  company_id IN (
    SELECT company_id FROM public.user_companies 
...
-- Integration logs: Users can access company logs
CREATE POLICY "Users can access integration logs" ON public.integration_logs
FOR ALL USING (
  company_id IN (
    SELECT company_id FROM public.user_companies 
    WHERE user_id = auth.uid() AND status = 'active'
...
-- Audit trail: Users can access company audit data
CREATE POLICY "Users can access audit trail" ON public.audit_trail
FOR ALL USING (
  company_id IN (
    SELECT company_id FROM public.user_companies 
    WHERE user_id = auth.uid() AND status = 'active'
...
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA kit TO authenticated;

-- Grant service_role full access
GRANT ALL ON ALL TABLES IN SCHEMA public TO service_role;
GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO service_role;
GRANT ALL ON ALL FUNCTIONS IN SCHEMA kit TO service_role;

/*
 * -------------------------------------------------------
 * Section: Post-Migration Setup
 * Add FK constraints if base tables exist
 * -------------------------------------------------------
 */

-- Add FK constraint to accounts table if it exists
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
        ...
        IF NOT EXISTS (
            ...
            WHERE constraint_name = 'companies_account_id_fkey' 
            AND table_name = 'companies'
        ) THEN
            ALTER TABLE public.companies 
            ADD CONSTRAINT companies_account_id_fkey 
